...
const OrderList = ({
            ...
            return (
                {
                    ...
                    accept: <div>
                        <button
                            onClick={() => { acceptOrder(client.name, order.ownerName, order.timestamp, order.dogName) }}
                            className="btn btn-success"
                            disabled={order.status !== "Posted"}>
                            Accept
                        </button>
                        <button
                            onClick={() => { refuseOrder(client.name, order.ownerName, order.timestamp, order.dogName) }}
    ...
    review.map((rev) => {
        ...
        reviewList.push(<div className='row'>
            <div className='col-md-4'>Owner Name:</div>
            <div className='col-md-8'>{rev.ownerName}</div>
        </div>)
        reviewList.push(<div className='row'>
            <div className='col-md-4'>Time:</div>
            <div className='col-md-8'>{Date(rev.timeStamp)}</div>
        </div>)
        reviewList.push(<div className='row'>
            <div className='col-md-4'>Rating:</div>
            <div className='col-md-7 btn btn-success' style={{ width: rate * 6 + "vw" }}>{Number((parseFloat(rev.rating)).toFixed(2))}</div>
        </div>)
        reviewList.push(<div className='row'>
            <div className='col-md-4'>Comment:</div>
            <div className='col-md-8'>{rev.comment}</div>
        </div>)
        reviewList.push(<hr />)
    })
    ...
    return (
        <section className="orderList row positionSetter">
            <div className='col-md-3 side'>
                <div onClick={() => setView(0)}>Order List</div>
                <hr></hr>
                <span onClick={() => setView(1)}>Account History</span><sup className='text-white' style={{ backgroundColor: "red", padding: "2px", borderRadius: "10px" }}>{client && client.pay}</sup>
                <hr></hr>
                <span onClick={() => setView(2)}>Reviews</span>
            </div>
